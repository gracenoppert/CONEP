...
local infile_stub "`state_name'WeeklyCasesByZipCode2022_rev"
...
gen day1_2020 = td(30dec2019)
format day1_2020 %td

gen day1_2021 = td(4jan2021)
format day1_2021 %td

gen day1_2022 = td(3jan2022)
format day1_2022 %td

gen day6_2020 = td(5jan2020)
format day6_2020 %td

gen day6_2021 = td(10jan2021)
format day6_2021 %td

gen day6_2022 = td(9jan2022)
format day6_2022 %td

destring(Week), gen(week_num) force
destring(Year), gen(year_num) force

gen date_start = day1_2020 + (week_num-1)*7 if year_num==2020
replace date_start = day1_2021 + (week_num-1)*7 if year_num==2021
replace date_start = day1_2022 + (week_num-1)*7 if year_num==2022
format date_start %td

gen date_end = day6_2020 + (week_num-1)*7 if year_num==2020
replace date_end = day6_2021 + (week_num-1)*7 if year_num==2021
replace date_end = day6_2022 + (week_num-1)*7 if year_num==2022
format date_end %td
...
capture drop zip_cases
egen zip_cases = total(cases), by(zip_code)
capture drop state_cases
