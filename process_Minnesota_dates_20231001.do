...
local infile_stub "`state_name'MonthlyCasesByZipCode2022"
...
rename date raw_date_str

gen month_str = regexs(1) if regexm(raw_date_str, "^([0-9]+)_([0-9]+)$")
gen year_str = regexs(2) if regexm(raw_date_str, "^([0-9]+)_([0-9]+)$")

destring(month_str), gen(month) force
destring(year_str), gen(year) force

gen date = .
replace date = mdy(month, 31, year) if inlist(month, 1, 3, 5, 7, 8, 10, 12)
replace date = mdy(month, 30, year) if inlist(month, 4, 6, 9, 11)
replace date = mdy(month, 28, year) if inlist(month, 2)
format date %td
